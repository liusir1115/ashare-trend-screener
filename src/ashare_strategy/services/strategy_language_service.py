...
SCOPE_LABELS = {"all": "全市场", "main_board": "主板", "star_market": "科创板", "growth_board": "成长板"}
STYLE_LABELS = {"breakout": "平台突破", "trend_start": "趋势启动", "rebound": "底部反转"}
HOLDING_LABELS = {"swing": "波段持有", "short": "短线快进快出", "trend": "趋势跟随"}
PRIORITY_LABELS = {"chip": "筹码结构", "volume": "放量突破", "base": "筑底时长", "valuation": "估值修复"}
...
def translate_reason(reason: str) -> str:
    exact_map = {
        ...
        "Base-building period is long enough": "筑底时长较充足",
...
def build_playbooks(strategy: Any) -> tuple[list[dict[str, Any]], str | None, str | None]:
    ...
    if any(keyword in text for keyword in ["反转", "反弹", "v反", "拐头"]):
        playbooks = [
            {"id": "reversal_base", "title": "底部反转型", "thesis": "更强调筑底、均线拐头和压力释放，不追求最强爆量。", "fit_for": "适合想做中短波段反转、能接受先试错后确认的人。", "overrides": {"style_focus": "rebound", "holding_period": "swing", "risk_tolerance": "balanced", "priority_signal": "base"}, "highlights": ["筑底 60-120 天", "量比 >= 1.3", "上方压力 <= 6%"]},
            {"id": "reversal_left", "title": "左侧试错型", "thesis": "更早介入，放松放量要求，但更看重筹码和空间。", "fit_for": "适合偏左侧、愿意用更紧止损换更早位置。", "overrides": {"style_focus": "rebound", "holding_period": "short", "risk_tolerance": "aggressive", "priority_signal": "chip"}, "highlights": ["筑底 40-90 天", "量比 >= 1.1", "止损偏紧"]},
            {"id": "reversal_right", "title": "右侧确认型", "thesis": "要求站回均线和放量确认，宁可晚一点，也更稳一点。", "fit_for": "适合不想猜底，想看到转强信号再上车。", "overrides": {"style_focus": "trend_start", "holding_period": "swing", "risk_tolerance": "conservative", "priority_signal": "volume"}, "highlights": ["量比 >= 1.8", "更看重均线和突破", "上方压力 <= 5%"]},
        ]
        recommended_id = "reversal_base"
    elif any(keyword in text for keyword in ["突破", "启动", "主升", "放量"]):
        playbooks = [
            {"id": "breakout_classic", "title": "经典突破型", "thesis": "以平台突破和放量确认为核心，先求确定性。", "fit_for": "适合你原本的趋势启动思路。", "overrides": {"style_focus": "breakout", "holding_period": "swing", "risk_tolerance": "balanced", "priority_signal": "volume"}, "highlights": ["量比 >= 1.8", "突破确认优先", "筑底 60-120 天"]},
            {"id": "breakout_value", "title": "估值修复突破型", "thesis": "在突破逻辑里，额外强调估值不过热和行业匹配。", "fit_for": "适合主板、偏低估修复的趋势启动。", "overrides": {"style_focus": "breakout", "holding_period": "trend", "risk_tolerance": "balanced", "valuation_weight": "high", "priority_signal": "valuation"}, "highlights": ["估值阈值更严", "持有更久", "行业热度仍需达标"]},
            {"id": "breakout_fast", "title": "短线爆量型", "thesis": "更强调爆量和短周期反馈，适合快进快出。", "fit_for": "适合强信号博弈，不适合太长持有。", "overrides": {"style_focus": "breakout", "holding_period": "short", "risk_tolerance": "aggressive", "priority_signal": "volume"}, "highlights": ["量比 >= 2.0", "持有 3-7 天", "更强调强信号"]},
        ]
        recommended_id = "breakout_classic"
    else:
        playbooks = [
            {"id": "balanced_default", "title": "平衡试探型", "thesis": "先用一套中性参数把策略跑通，再决定往哪边加码。", "fit_for": "适合描述还比较模糊时先验证方向。", "overrides": {}, "highlights": ["风险平衡", "波段持有", "优先主板验证"]},
            {"id": "chip_first", "title": "筹码优先型", "thesis": "先把上方压力和筹码干净度放在第一位。", "fit_for": "适合你原本强调上方筹码干净的思路。", "overrides": {"priority_signal": "chip", "risk_tolerance": "conservative"}, "highlights": ["上方压力更严", "筹码集中度更重要", "容忍更少噪音"]},
            {"id": "trend_first", "title": "趋势确认型", "thesis": "先确认均线、斜率和量能，再决定是否上车。", "fit_for": "适合不想太主观猜底，更看重右侧确认。", "overrides": {"style_focus": "trend_start", "priority_signal": "volume", "risk_tolerance": "conservative"}, "highlights": ["均线多头优先", "放量确认更严", "适合右侧交易"]},
        ]
