...
def answer_question(question: str, strategy: StrategyInput) -> dict[str, Any]:
    ...
    if _has_any(text, ["高切低", "切换", "资金流向", "资金去哪里", "资金"]):
    ...
    return _answer("这个免费问答目前只覆盖策略理解、候选原因、资金流、高切低和热点风险。更开放的发散问答后面需要接大模型。", payload)
...
def _answer_candidate(symbol: str | None, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if candidate is None:
        return _answer("当前没有候选结果可以解读。", payload)
...
def _answer(answer: str, payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "answer": answer,
        "mode": "规则问答",
        "suggestions": ["为什么第一只票被淘汰？", "今天有没有高切低？", "反转策略怎么量化？", "当前有什么风险？"],
