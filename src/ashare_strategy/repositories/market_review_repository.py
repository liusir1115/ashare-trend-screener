...
def _read_sector_flows(loader: Callable[[], Any], source: str) -> list[SectorFlow]:
    ...
    for row in frame.to_dict(orient="records"):
        ...
        flows.append(
            SectorFlow(
                name=name,
                change_pct=_as_float(_pick(row, "涨跌幅", "今日涨跌幅")),
...
def _read_news_items(loader: Callable[[], Any], source: str, limit: int = 8) -> list[NewsItem]:
    ...
    for row in frame.to_dict(orient="records"):
        ...
        if not title:
            continue
        items.append(NewsItem(title=title[:120], time_text=str(_pick(row, "时间", "发布时间", "日期") or ""), tag=_news_tag(title), source=source))
        if len(items) >= limit:
            break
    return items
...
def _serialize_news(item: NewsItem) -> dict[str, str]:
    return {"title": item.title, "time": item.time_text, "tag": item.tag, "source": item.source}
...
def _build_news_summary(news_items: list[NewsItem], board_changes: list[NewsItem]) -> dict[str, Any]:
    ...
    headline = "热点集中在：" + "、".join(tag for tag, _count in hot_tags) if hot_tags else "暂未抓到明显热点。"
...
def _serialize_flow(flow: SectorFlow) -> dict[str, Any]:
    return {"name": flow.name, "change_pct": round(flow.change_pct, 2), "net_inflow": round(flow.net_inflow, 2), "net_inflow_text": _format_money(flow.net_inflow), "source": flow.source}
...
def _build_flow_summary(industry_flows: list[SectorFlow], concept_flows: list[SectorFlow]) -> dict[str, Any]:
    ...
    low_level_inflow = [item for item in inflow if item.net_inflow > 0 and item.change_pct <= 1.5]
    high_level_outflow = [item for item in outflow if item.net_inflow < 0 and item.change_pct >= 1.5]
    ...
    return {
        ...
        "rotation": {"summary": rotation_text, "low_level_inflow": [_serialize_flow(item) for item in low_level_inflow[:3]], "high_level_outflow": [_serialize_flow(item) for item in high_level_outflow[:3]]},
    }
...
def _demo_news_summary() -> dict[str, Any]:
    return {
        ...
        "hot_tags": [{"name": "政策", "count": 3}, {"name": "科技", "count": 2}, {"name": "商品", "count": 2}],
        "items": [
        ...
        "risks": [{"title": "高位题材出现资金流出，短线追高风险增加", "time": "盘后", "tag": "风险", "source": "演示快讯"}],
        "candidate_links": [],
        "candidate_note": "这里会结合候选池补充更值得联动观察的股票。",
    }
...
def build_market_review() -> dict[str, Any]:
    ak = _require_akshare()
    industry_flows = _read_sector_flows(lambda: ak.stock_sector_fund_flow_rank(indicator="今日", sector_type="行业资金流"), "行业资金流")
    concept_flows = _read_sector_flows(lambda: ak.stock_sector_fund_flow_rank(indicator="今日", sector_type="概念资金流"), "概念资金流")
    ...
    summary = _build_flow_summary(industry_flows, concept_flows) if flow_live_data else _build_flow_summary(
        [SectorFlow("电力", 0.8, 1_620_000_000, "演示行业资金"), SectorFlow("煤炭", 1.1, 980_000_000, "演示行业资金"), SectorFlow("算力", 3.4, -1_250_000_000, "演示概念资金"), SectorFlow("半导体", 2.2, -860_000_000, "演示行业资金"), SectorFlow("银行", 0.4, 720_000_000, "演示行业资金")],
        [SectorFlow("中特估", 0.9, 890_000_000, "演示概念资金"), SectorFlow("低空经济", 4.2, -930_000_000, "演示概念资金"), SectorFlow("储能", 1.3, 620_000_000, "演示概念资金")],
    )
    ...
    if flow_live_data and news_live_data:
        ...
        headline = "演示复盘：资金偏向低位防守和能源方向，高位题材有分歧。"
        note = "当前为演示资金流和演示快讯，用来保证页面流程稳定。"
    summary.update({"trade_date": date.today().isoformat(), "live_data": flow_live_data and news_live_data, "flow_live_data": flow_live_data, "news_live_data": news_live_data, "headline": headline, "note": note, "news": news_summary})
    return summary


def fetch_market_review(*, force_fallback: bool) -> dict[str, Any]:
    if force_fallback:
        return build_demo_market_review()
    try:
        return build_market_review()
    except Exception:
        return build_demo_market_review()
