...
def _build_source_status(results: dict[str, Any], market_review: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "items": [
            ...
            {
                "name": "新闻快讯",
                "state": "真实 AKShare" if news_is_live else "演示兜底",
                "detail": "用于盘前盘后看热点、政策、海外风险和新闻摘要。",
                "ok": news_is_live,
            },
            {
                "name": "资金流复盘",
                "state": "真实 AKShare" if flow_is_live else "演示兜底",
                "detail": "用于盘后看行业/概念资金和高切低线索，免费接口偶尔会抽风。",
                "ok": flow_is_live,
            },
            {
                "name": "策略问答",
...
def _build_news_candidate_links(results: dict[str, Any], news_summary: dict[str, Any]) -> dict[str, Any]:
    tag_keywords = {
    ...
    candidate_pool = [
        ...
        if item.get("passed_filters") or float(item.get("score", {}).get("total", 0)) >= 55
    ][:5]
    ...
    for item in candidate_pool:
        ...
        candidate_text = " ".join(
            [
                str(metrics.get("limit_reason", "")),
                " ".join(item.get("score", {}).get("reasons", [])),
                " ".join(item.get("failed_reasons", [])),
            ]
        )
        matched_tags = [
            tag for tag in hot_tags
            if any(keyword in candidate_text for keyword in tag_keywords.get(tag, []))
        ]
        if matched_tags:
        ...
        linked_candidates.append(
            {
                ...
                "status": "通过" if item.get("passed_filters") else "观察",
                "note": note,
            }
        )
    ...
    if linked_candidates:
        ...
        candidate_note = "当前候选池还没有特别贴近热点的票，先以新闻摘要为主。"

    return {
        **news_summary,
        "candidate_links": linked_candidates[:3],
...
def build_mvp_payload(strategy: StrategyInput) -> dict[str, Any]:
    ...
    market_review["news"] = _build_news_candidate_links(results, market_review.get("news", {}))
    return {
