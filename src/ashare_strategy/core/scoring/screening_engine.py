...
from ashare_strategy.core.models import DailySnapshot, ScoreBreakdown, ScreeningResult
...
class ScreeningEngine:
    ...
    def _hard_filters(self, snapshot: DailySnapshot) -> list[str]:
        ...
        if snapshot.chip_compact > chip.max_chip_compact:
            failed.append(f"chip_compact={snapshot.chip_compact:.2%} above {chip.max_chip_compact:.2%}")
        ...
        if snapshot.ma20_slope < technical.min_ma20_slope:
            failed.append(f"ma20_slope={snapshot.ma20_slope:.4f} below {technical.min_ma20_slope:.4f}")
        if snapshot.ma60_slope < technical.min_ma60_slope:
            failed.append(f"ma60_slope={snapshot.ma60_slope:.4f} below {technical.min_ma60_slope:.4f}")
        ...
        if snapshot.volume_ratio < technical.min_volume_ratio:
            failed.append(f"volume_ratio={snapshot.volume_ratio:.2f} below {technical.min_volume_ratio:.2f}")
        ...
        if snapshot.close_to_high_250 > base.max_close_to_high_250:
            failed.append(f"close/high_250={snapshot.close_to_high_250:.2f} above {base.max_close_to_high_250:.2f}")
        ...
        if snapshot.board_hot_score < valuation.min_board_hot_score:
            failed.append(f"board_hot_score={snapshot.board_hot_score:.2f} below {valuation.min_board_hot_score:.2f}")
    ...
    def _score(self, snapshot: DailySnapshot) -> ScoreBreakdown:
        ...
        chip_parts = [
            _score_inverse(snapshot.overhead_pressure, chip_cfg.excellent_overhead_pressure, chip_cfg.max_overhead_pressure),
            _score_inverse(snapshot.chip_compact, chip_cfg.excellent_chip_compact, chip_cfg.max_chip_compact),
        ...
        technical_score = _clamp((ma_alignment_score + breakout_score + volume_score + slope_score) / 4.0 + limit_up_bonus)
