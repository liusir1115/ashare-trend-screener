...
from ashare_strategy.core.models import BacktestReport, DailyBar, DailySnapshot, TradeRecord
...
class BacktestEngine:
    ...
    def run(
        ...
        bars_index = {symbol: sorted(bars, key=lambda item: item.trade_date) for symbol, bars in bars_by_symbol.items()}
        bars_lookup = {symbol: {bar.trade_date: index for index, bar in enumerate(series)} for symbol, series in bars_index.items()}
